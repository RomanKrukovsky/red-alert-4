...
| **I-B5** | ~~Replay-reconstructible belief test~~ — **DONE 2026-08-06** | `Intel.BeliefIsReplayReconstructible`: two independent SimWorlds fed the identical recorded command stream agree on the full state checksum (which includes every PerceivedWorld) at all 120 ticks AND on the exact track contents at the end. Closes INVARIANT 11. |
