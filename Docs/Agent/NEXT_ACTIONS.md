...
| **V-7** | Fog-of-war 3D rendering (ADR-0028) — **post-process pass + visual check OWED** | Landscape tinting is implemented and building: `FogVisibilityTexture.h` encoding (headless-tested), per-frame upload with dirty-region path in `URA4SimWorldSubsystem`, fog nodes generated into `M_RA4_TerrainLayered` by the commandlet so the next run cannot revert them. **Not done**: (a) the post-process pass — without it, props, water and buildings stay lit over unexplored black ground, which ADR-0028 lists as a known gap and is required before this counts as complete; (b) high-contrast fog mode and the fog-strength floor from ADR-0028 §4; (c) **nobody has launched the editor and looked at it** — same visual debt as V-A, must not be inherited silently. Verified so far: headless 451/451, UE 5.8 editor target `Result: Succeeded`. |
