...
| **I-M3-perf** | Spatial index for belief queries (review M2, deferred) | `ApplyGroupedReport`'s nearby-track search is O(TrackCapacity) per group and the GC association sever is O(EntityCapacity) per collected track; a decay avalanche (blackout raises decay for every track at once, so mass GC is the designed-for case) can reach tens of millions of comparisons in one tick. `GetTracksInRegion` is still a linear scan, and `PerceivedWorld.cpp` promised M3 would repay that debt. Needs a spatial grid plus a TrackId→slot reverse index, measured at the 2000-entity baseline. Gate for I-M5. |
...
| **I-M5-scenario** | AttackMove does not move units in the ProvingGround 4-player scenario | Found while fixing the recon budget benchmark: 2,000 units issued `AttackMove` to the map centre on the first tick do not move at all — instrumented, the first unit was still at its exact spawn point (600,600) after **1,200 ticks** (60 s of sim time). Commands are built correctly (owner, unit kind, valid entity) and admission accepts them, so the defect is downstream in order execution or pathfinding for this scenario. Not a recon bug: `ProvingGround.BenchmarkScenarioActuallyFormsBelief` passes with identical recon settings, and belief forms in 30 ticks once two units are placed in contact. The benchmark was rewritten to spawn armies already in contact so it measures recon rather than movement — that removes the false-green, it does NOT fix this. Reproduce by placing the four blocks 5,200 units apart again and printing a unit position after the warm-up. |
| **I-M5-inflight** | `Recon.SaveLoadPreservesReportsStillInFlight` fails | "live and restored worlds diverged 1 ticks after load: an in-flight report did not survive" (TestRecon.cpp:1271). Not caused by the benchmark work — it became **observable** only when the build started linking again: 5ccc1cd left `GetConstructionProgressPerMille` declared but undefined, so `RA4Tests` did not link and no test result from that commit onward was real. Anyone reading a green status between 5ccc1cd and the fix was reading nothing. Owner: whoever owns recon save/load (M3). Reproduce: build headless and run `RA4Tests`; it is deterministic, failing on the first tick after load. |
