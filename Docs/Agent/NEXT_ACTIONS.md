...
| **V-4** | ~~Repath CityPark material references~~ **DONE — verified by running 2026-08-06** | Fixed by two `CoreRedirects` substring entries in `Config/DefaultEngine.ini` (QuantumCharacter and CityPark), not by touching assets. Evidence: `UnrealEditor-Cmd RedAlert4.uproject /Game/Maps/RA4_Skirmish_Production -run=ResavePackages -verify -unattended -abslog=/tmp/v4verify.log` — 3.4 MB log, map loaded (7 references), **zero `]Error:` lines and zero material/texture/mesh load failures**. The single `LoadErrors:` line is unrelated (`GameFeatureData` asset-manager config). Remaining CityPark mentions are two benign warnings: the documented `MatchSubstring` deprecation notice, and `TConvex` physics warnings from an unused pack showcase map. Caveat on method: a first attempt appeared to pass with "0 errors" because `timeout` does not exist on macOS, so the command never ran — the number came from an empty pipe. Re-run with an explicit `-abslog` for real evidence; do not trust a zero from a command whose exit status was not checked. |
