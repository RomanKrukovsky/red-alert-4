...
| :--- | :--- | :--- |
| **CI-1** | ~~Reconcile the two provenance formats~~ — **DONE 2026-08-06** | Registry converted to ADR-011's schema (it is the release gate, so it wins), with subtree records and tree-wide digests. Records state what things actually are: generated_from_internal for EVA voice / blockout / maps / UI, cleared third_party for ambientCG and the OFL fonts, and `release_allowed=false` + pending review for QuantumCharacter, IndustryPropsPack6 and the Epic/Quixel placeholders. Commit `981c237`. |
| **CI-2** | ~~Close the remaining findings~~ — **DONE 2026-08-06** | 68 workstation paths rewritten out of content and docs (`11f5159`); the shipping `DefaultGame.ini` description and a `ControlScheme.h` comment rewritten rather than suppressed, because they ship; 35 single-file suppressions with reviewer and expiry for the legal audits, ADR-011 and the historical gap analyses. A first attempt used a blanket `Docs/**` glob — the scanner's own corpus test caught it and failed, so the globs were narrowed to named files. Scanner: **[PASS] 0 violations**. Commit `bbc2f40`. |
| **CI-3** | Prove the determinism gate actually runs | CI-1 and CI-2 are now green locally, so the gate can clear. Confirm from a real run that `Cross-Platform Determinism Verification` compares three checksums and that they match. This has never happened; until it does, cross-platform determinism is an assumption. |
...
| **P-3** | ~~Consolidate duplicate ADR numbering~~ — **DONE 2026-08-20** | Done: all 23 files in the legacy `Docs/ADR/` and `Docs/ADRs/` directories now carry SUPERSEDED banners (ADR-0011 decided this and it had never been applied — no file had any marker); `Docs/integration/templates/ADR_INDEX.md` is quarantined with a banner because it reserved numbers 0012–0021 that are *all* already occupied in the canonical series, so any agent following it would have created ten collisions; ADR-0011's own errata are recorded in it. Recovered `ADR-0027-faction-economy-extension-points.md` (authored as ADR-0018 on the dead branch, colliding with `main`'s ADR-0018 "No LLM In The Runtime Command Path"). **Closed 2026-08-20**: the DirectControl ADR is renumbered to **ADR-0029** (`ADR-0029-direct-control.md`), not the ADR-0028 prescribed here — 0028 was claimed by the telemetry ADRs while this item sat open, which is itself the lesson: a number reserved in a tracker is not reserved in the directory. A redirect stub stays at `ADR-0011-DirectControl.md`. Inbound citations updated in `RA4DirectControlSubsystem.h`, `RA4DirectControlProfile.h`, `Command.h` (it pointed at `ADR-DirectControl.md`, a path that never existed), ADR-0022 and ADR-0026. ADR-0011 now means only the consolidation decision. Next free canonical number is **ADR-0030**. One filename-level duplicate remains and is deliberately left: the two ADR-0028 telemetry files are an architecture decision and its implementation plan, not two decisions. |
