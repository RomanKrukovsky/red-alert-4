...
| **V-7** | Fog-of-war 3D rendering (ADR-0028) — **code complete; in-editor visual check OWED** | Done: texel encoding (`FogVisibilityTexture.h`, headless-tested), per-frame upload with dirty-region path, terrain fog nodes and a **fog post-process material** both generated by `RA4LayeredTerrainSetupCommandlet` so neither can be reverted by its next run, post-process installed on the camera pawn as a weighted blendable (this is what fogs props, water and buildings — without it they stayed lit over unexplored black ground), plus `SetFogStrength`/`SetHighContrastFog` from ADR-0028 §4 with the strength floor enforced in code (`kMinFogStrength = 0.35`) and pinned by a test. Verified: headless 452/452, UE 5.8 editor target `Result: Succeeded`. **Still owed and NOT claimed**: (a) nobody has launched the editor and looked at fog on screen — same debt as V-A, both must be discharged together in one editor session; (b) the generated `M_RA4_FogPostProcess` asset does not exist until someone runs the commandlet in this branch — until then the camera logs an error once and the landscape-only fog stands; (c) fog boundary softness at map edges and over water is unassessed because it can only be judged visually. |
