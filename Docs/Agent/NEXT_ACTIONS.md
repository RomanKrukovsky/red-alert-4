...
| Task ID | Task | Acceptance criteria |
| :--- | :--- | :--- |
| **I-B1** | Remove ground truth from the belief read surface | `PerceivedTrack::bPhantom` (`IntelTypes.h:120`) is a member of the struct returned by `PerceivedWorld::GetTracksInRegion` (`PerceivedWorld.h:49`), so any UI/AI caller can read whether a contact is fabricated. Move it to a core-internal side table beside the track↔entity association. Add an instrumented leak detector that fails the build if any ground-truth field is reachable from the read surface. Closes INVARIANT 10 and ADR-0021 verification item 2. |
| **I-B2** | Make belief write access structural | `IntelSystem::GetPerceivedWorldMutable(PlayerId)` (`IntelSystem.h:80`) and `PerceivedWorld::SetLastObservedTick` (`PerceivedWorld.h:56`) let anything holding the system mutate any player's belief. Put both behind a private writer interface (friend `IntelSystem`) or split off a const-only view type. Closes INVARIANT 9. |
| **I-B3** | Decide the fate of per-source-type decay | ADR-0021 requires decay rates that differ by source type (radar faster than visual); `PerceivedTrack` has no source type, only `IndependentSourceCount`, and `TrackTuning` has one global rate. Either restore a source-type field with per-type curves, or amend ADR-0021 section 3 with the reason a single curve suffices. Currently the requirement is silently unimplementable. |
...
| Task ID | Task | Acceptance criteria |
| :--- | :--- | :--- |
| **P-8** | Fix ADR-0022's latency budget and close its two deferred decisions | The reviewer found the "≤ 12 ticks (200 ms)" figure was computed at 60Hz — at 20 Hz that is 600 ms, three times the claim, before lockstep input delay. Already corrected to 4 ticks in the ADR; the budget must now be restated to *include* input delay. Also: the order-supersede rule is described as "need a rule" rather than specified, and CommandGraph edge recompute has no defined tick phase or traversal order — both are desync sources and both must be pinned before Accepted. (DirectControl interaction is already resolved.) |
| **P-9** | Correct ADR-0023's scope claim and draw its boundary with ADR-0026 | "Pure data + KnowledgeMap writes — no new engine systems" understates the work: resolving sensor type against `SignatureProfile` per observer per target is a new sensor model with O(observers × targets) cost, i.e. an Intel-layer rework. Restate the estimate. Add ADR-0022 to `Depends on` (the ADR already references CommandGraph detection). Define whether phantom contacts occupy navigation/reservation cells and how they count against entity caps. Delimit responsibility against ADR-0026's fabrication stage so the two distortion systems do not overlap. |
| **P-10** | Give ADR-0024 a nav-invalidation contract | Improvised roads mutate movement cost mid-match while flow fields are built from cached cost. "Needs determinism tests" is insufficient: specify the tick phase at which cost changes batch, the deterministic order of field recompute, and the per-tick recompute budget. Also specify incremental/dirty-region hashing for TerrainStateLayer, since it is hash- and checkpoint-relevant at full grid size. |
| **P-11** | Specify DoctrineBias serialization and its relationship to existing AI types | For "same replay + same DoctrineBias ⇒ identical hash" to hold cross-platform, the blob needs a canonical encoding: key order for `MapRoutePriors`, `Fixed` (never float) weights, behaviour on unknown `mapId`, and reader behaviour on an older `ProfileVersion`. Separately: `OpponentModel.h`, `AIDoctrine.h` and `AIStrategy.h` already exist in `RA4AI` — state whether ADR-0025 extends them or the ADR is creating a duplicate subsystem, which project rules forbid. Remove the claim that a digital twin is a "free by-product": imitating a player's style needs behavioural fidelity, not reweighted utility scores, and warrants its own ADR. |
| **P-12** | Add the unaddressed design risks to the ADR Consequences sections | Reviewers flagged risks absent from the ADRs' own Consequences: profile poisoning (a player deliberately teaching the AI false habits to farm useless counters); hostility to newcomers (strong bias inferred from 3–5 matches at low difficulty); the transparency screen simultaneously teaching players how to game the AI; wreck-spam as a griefing vector to flush valuable enemy wrecks out of the cap; salvage economy rewarding the attacker for losing units, weakening the punishment for bad attacks; improvised roads collapsing the map into a single optimal highway; and spectator readability collapsing under mass fake contacts (conflicts with ADR-0010 spectator delay). Each ADR records the risks that belong to it, with a mitigation or an explicit accepted-risk note. |
