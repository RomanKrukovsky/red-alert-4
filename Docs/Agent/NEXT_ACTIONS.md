...
| **V-7** | Fog-of-war 3D rendering (ADR-0028) — **assets generated and parameter-verified; in-editor visual check still OWED** | Terrain fog + post-process both generated by the commandlet and confirmed on disk (`RA4LayeredTerrain: fog-of-war nodes wired into BaseColor` / `fog post-process material generated`, zero errors in the run). **A real defect was caught doing this**: the code set four material parameters and the generated materials declared only `RA4FogWorldWidth`, so fog strength, the high-contrast accessibility mode and the vertical UV axis did nothing at all — Unreal ignores a set for a parameter that does not exist, silently, with no error anywhere. All four now exist in both materials, wired through contrast and strength mixes, and `Tools/Editor/verify_fog_materials.py` pins the C++-literal-to-generated-asset contract that no compile or headless test can see (run it after any fog parameter change; it exits non-zero on mismatch). Non-square maps also fogged correctly only after adding the height axis. **Still owed**: nobody has looked at fog on screen, so boundary softness at map edges and over water remains unassessed — that and V-A's visual debt must be discharged in one editor session. Note: the generated `.uasset` files are gitignored under `/Content/RA4/Generated/`, so a fresh clone must run the commandlet — that is the intended workflow, not a gap. |
