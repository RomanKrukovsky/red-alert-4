# ADR-0027: Faction Economy Extension Points

> **RENUMBERED from ADR-0018 (2026-08-05).** This document existed only on
> `remediation/foundation-fixes`, a branch that does not compile and has been superseded by `main` —
> and its original number, ADR-0018, is occupied on `main` by "No LLM In The Runtime Command Path".
> Recovered here under the next free number so that the decision is not lost with the branch and no
> collision is introduced. References to "ADR-0018 (Faction Economy Extension Points)" elsewhere —
> notably in ADR-0011's consolidation decision 4 — mean this document.
>
> Tick-derived figures below were authored when the docs claimed 60Hz; the real rate is 20 Hz
> (`kTicksPerSecond`, `SimConfig.h`). Verify any tick constant here before implementing.
