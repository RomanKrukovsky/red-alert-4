...
| `RA4AutoPlayDriver.cpp` | **OWN + FOG-GATED** | Five build/economy/attack-unit scans filter to the local player's entities. `FindEnemyBasePosition` previously read hidden enemy truth; it now accepts only contacts passing `IsEntityVisibleTo(LocalPlayer(), …)`. The driver is opt-in test automation, but it still follows the same visibility boundary as a player. |
...
| V-B | ~~Fix `RA4PlayerController.cpp` picking~~ **DONE**: `BuildPickCandidates` skips entities failing `IsEntityVisibleTo(Selection.GetLocalPlayer(), …)`, so cursor, hover tooltip and click-pick can no longer land on fogged enemies. | done | Unreal Integration |
...
| V-F | ~~Combat-event debug VFX leak~~ **DONE**: all three draw sites in `RA4SimWorldSubsystem` are gated — death markers, weapon tracers and impact splashes. Needed a new `SimWorld::IsLocationVisibleTo(Viewer, Vec2)` because combat events carry a *location*, not a live entity (the shooter may already be dead when presentation reads the event). The tracer gates on BOTH ends: a line between two positions leaks whichever end the player cannot see, so the shot draws only when the shooter's tile is visible and the line only when the target is visible too — partial visibility degrades to a muzzle flash. Pinned by `FogOfWar.LocationVisibilityGateMatchesEntityGate`, which also documents the deliberate asymmetry: the entity gate short-circuits on ownership, a location has no owner, so off-map points are visible to nobody. | done | Unreal Integration |
| V-G | ~~Extend the static detector to `IsEntityVisibleTo`~~ **DONE**: `Recon.ObjectiveStateFunnelInventory` now watches three symbols in two groups — bulk readers (`GetAllCores`/`GetAllTransforms`) and the visibility gate (`IsEntityVisibleTo`), each with its own short whitelist, because they leak differently: the first hands out the whole objective world, the second answers "can viewer V see entity E" for an **arbitrary** viewer. A new file touching either fails the build with symbol-specific guidance until classified here. Added `Recon.ObjectiveStateFunnelWhitelistsAreNotStale`, which fails when a whitelisted file **stops** reading its symbol — dead permission silently re-authorizes the next author. It immediately found three: `AIDebugOverlay.cpp` and `ReconSystem.cpp` no longer call the bulk readers at all, and `SimWorld.cpp` works on the private `Core` member rather than its own getter. All three dropped. | done | QA |
